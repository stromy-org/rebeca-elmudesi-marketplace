# Rebeca Elmudesi Plugin Marketplace

Public marketplace for Rebeca Elmudesi Claude Code plugins.
...
2. Click the `+` to add a marketplace → enter `stromy-org/rebeca-elmudesi-marketplace`
3. Click **Rebeca Elmudesi** → Install

### Option B: From the CLI

```bash
# Add marketplace (one-time)
claude plugin marketplace add stromy-org/rebeca-elmudesi-marketplace

# Install plugin
claude plugin install rebeca-elmudesi-plugin@rebeca-elmudesi-marketplace
```

### Post-install: dependencies (one-time)

```bash
cd ~/.claude/plugins/cache/rebeca-elmudesi-marketplace/rebeca-elmudesi-plugin/0.1.0
...
| Skill | Command |
|-------|---------|
| _example_ | `/rebeca-elmudesi-plugin:example` |

## Updating

```bash
claude plugin update rebeca-elmudesi-plugin@rebeca-elmudesi-marketplace
```
...
| "Plugin not found in marketplace" | Run `claude plugin marketplace add stromy-org/rebeca-elmudesi-marketplace` first |

## Architecture

- **Marketplace** (this repo): public — hosts `marketplace.json` only
- **Plugin** (`rebeca-elmudesi-plugin`): public — contains skills, brand data, company info
- **Source format**: use `"source": "url"` with an explicit HTTPS clone URL ending in `.git`, for example `https://github.com/stromy-org/rebeca-elmudesi-plugin.git`
